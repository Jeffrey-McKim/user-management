<?php
//session will send non logged in users to login page
session_start();
require_once('../connect.php');
if(!isset($_SESSION['username']) & empty($_SESSION['username'])){
  header('location: ..\login.php');
}
$username = $_SESSION['username']; //stores the username of the logged in user in $username
if(isset($_POST) & !empty($_POST)){
  if(isset($_POST['firstname']) & isset($_POST['lastname'])){
    if(!empty($_POST['firstname']) & !empty($_POST['lastname'])){
      $firstname = $_POST['firstname']; //stores user input into $firtname & $lastname variables
      $lastname = $_POST['lastname'];
        $usql = "UPDATE `usermanagement` SET firstname='$firstname', lastname='$lastname' WHERE username='$username'"; // updates the database with first and last names inputed by the user
  ...
  if(isset($_POST['username']) & !empty($_POST['username'])){ //if ths username already is the name of the current user it will relay this message
      echo "Not Possible to Update Username";
    }
  }

  if(isset($_POST['email']) & !empty($_POST['email'])){ //checks database for indentical email, then updates it if one is not found
    $email = $_POST['email'];
...
<div class="col-sm-9">
  <?php
    $sql = "SELECT * FROM `usermanagement` WHERE username='$username'"; // still dont know what this does but im afraid to delete it
